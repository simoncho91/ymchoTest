...
//var akc_url = "/search_utf8/common/module/akc.jsp";
var akc_url = "/por/sc/por_sc_akc_result.do";
...
function akc_init() {
	...
	if (akc_enable == 1) {
		onofftextobj.innerHTML = "꺼짐";
	}
	else {
		onofftextobj.innerHTML = "켜짐";
	}
	...
	AKCKwd_X = getElementX(parent.document.getElementById(AKCKwdID));  //검색창의 X좌표
    AKCKwd_Y = getElementY(parent.document.getElementById(AKCKwdID)) - 84;  //검색창의 Y좌표 
    
    AKCKwd_W = parent.document.getElementById(AKCKwdID).offsetWidth;   //검색창의 넓이
    AKCKwd_H = parent.document.getElementById(AKCKwdID).offsetHeight;  //검색창의 높이


	AKC_Arrow.style.top = AKCKwd_Y + 4 + "px";
	AKC_Arrow.style.left = AKCKwd_X + parseInt(AKCKwd_W) - 22 + "px";

	AKC_Div.style.top = AKCKwd_Y +  AKCKwd_H - 1 + "px";
	AKC_Div.style.left = AKCKwd_X  + "px";
...
function akc_curstyle(ob,b) {
	...
	if(ob) {
		...
		ob.style.backgroundColor = "#99CCFF";
...
//GET Method
		...
		//akc_request.open("GET", akc_url + "?q=" + encodeURI(q) + "&s=" + d + "&nflag=" + nflag, true);
akc_request.open("GET", akc_url + "?q=" + encodeURIComponent(q) + "&s=" + d, true);
		//alert(q);
...
function akc_chgimg(d) {
	...
	if(d==0) {
		akc_leftimg.src = "/IMG/KO/sc/btn_left_on.gif";
		akc_rightimg.src = "/IMG/KO/sc/btn_right_off.gif";
	} else if(d==1) {
		akc_leftimg.src = "/IMG/KO/sc/btn_left_off.gif";
		akc_rightimg.src = "/IMG/KO/sc/btn_right_on.gif";
	} else if(d==2){
		akc_leftimg.src = "/IMG/KO/sc/btn_left_off.gif";
		akc_rightimg.src = "/IMG/KO/sc/btn_right_off.gif";
	}
...
function akc_chgbtn(bool) {
	...
	if(akc_enable==1) {
		if(bool) {
			AKC_Arrow.src = "/IMG/2013/KO/common/btn_down.gif";
		} else {
			AKC_Arrow.src = "/IMG/2013/KO/common/btn_down.gif";
		}
	} else {
...
function akc_chklist(i) {
	...
	if(i==1) {
		akc_msg = "<div style='padding-top:3px; padding-left:5px; line-height:180%;font-face:굴림;font-size: 9pt'><font color='#0000FF'>검색어 자동완성 기능</font>이 켜져있습니다</div>";
	} else {
		akc_msg = "<div style='padding-top:3px;padding-left:5px; line-height:180%;font-face:굴림;font-size: 9pt'><font color='#0000FF'>검색어 자동완성 기능</font>이 꺼져있습니다</div>";
	}
...
function akc_chgturnimg(bool) {
	...
	if(bool) {
		akc_imgchk.src = "/IMG/KO/sc/btn_turn_off.gif";
	} else {
		akc_imgchk.src = "/IMG/KO/sc/btn_turn_on.gif";
	}
}
...
function fnChangeRadio(obj, event) {
	var objRadio = document.getElementsByName("nflag");
	var check = "";
	for(var i=0; i<4; i++) {
		if(objRadio[i].checked) {
			check	=	objRadio[i].value;
		}
	}
	
	var frm	=	parent.document.searchForm;
	
	frm["nflag"].value = check;
	akc_handle(event);
	akc_req2();
}