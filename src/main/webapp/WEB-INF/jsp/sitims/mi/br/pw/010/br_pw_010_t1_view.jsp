...
<script type='text/javascript'>
 j$(function(){
	 	...
	 	$('.btn_reqItem_list').click(function(){			
			var _patn = WebPATH + "br/pw/020/br_pw_020_req_modi_list_pop.do?i_sRecordId=" + j$("input[name='i_sRecordId']").val() +"&i_sProductCd="+j$("input[name='i_sProductCd']").val()+"&i_sFlag=KO";
	 	...
	 	$('.btn_modify_pop').click(function(e){
	 		...
	 		fn_pop({url:'/br/pr/010/br_pr_010_modify_pop.do?i_sProductCd='+product_id+'&i_sRecordId='+record_id ,title:'국내검토[기본정보] 수정',width:'1000',height:'400'});
...
//	 					float:right;
	 	...
	 	$('.btn_referProdPop').click(function(e){
	 		...
	 		var url ='/br/pw/010/br_pw_010_t1_view.do?i_sRecordId='+record_id+'&i_sProductCd='+product_id+'&i_sPopUpYn=Y';
...
<%-- 			<c:forEach items="${prodList}" var="vo" varStatus="s"> --%>
				<tbody class="tbody_product_info">
					...
					<tr>
						...
						<td>
							<div style="margin-bottom: 5px;">
								용량 : <span>${rVo.v_capacity }</span>
							</div>
							<div>
								<span style="float: left;">용기형태 :&nbsp; ${rVo.v_cntr_form_nm }</span>						
					...
					<tr class="tr_origin_img">
						<th>사용방법</th>
						<td colspan="4">
							<CmTagLib:cmAttachFile uploadCd="PON" type="viewLog" recordId="${prodVo.v_record_id}" pk1="${prodVo.v_product_cd}" />
							<span><pre>${rVo.v_pon_msg }</pre></span>
						</td>
					</tr>
				</tbody>
<%-- 			</c:forEach> --%>
