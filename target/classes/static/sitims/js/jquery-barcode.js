...
(function ($) {
  ...
  var barcode = {
    ...
    digitToCssRenderer : function($container, settings, digit, hri, mw, mh){
      ...
      if (settings.showHRI){
    	// 2018.09.18 By soheej -을 포함한 바코드 생성 시 인식율 저하로 인해 -을 삭제한 상태로 출력하고 hri 부위는 -을 강제 삽입
    	  var realHRI = "";
    	  if (hri.length == 13){
    		  realHRI = hri.substr(0, 2) + '-' + hri.substr(2, 3) + '-' + hri.substr(5, 5)+ '-' + hri.substr(10, 3);
    	  }else{
    		  realHRI = hri.substr(0, 3) + '-' + hri.substr(3, 5) + '-' + hri.substr(8, 3)+ '-' + hri.substr(11, 3);
    	  } 
//        content += "<div style=\"clear:both; width: 100%; background-color: " + settings.bgColor + "; color: " + settings.color + "; text-align: center; font-size: " + settings.fontSize + "px; margin-top: " + settings.marginHRI + "px;\">"+hri+"</div>";
    	  content += "<div style=\"clear:both; width: 100%; background-color: " + settings.bgColor + "; color: " + settings.color + "; text-align: center; font-size: " + settings.fontSize + "px; margin-top: " + settings.marginHRI + "px;\">"+realHRI+"</div>";
      }
